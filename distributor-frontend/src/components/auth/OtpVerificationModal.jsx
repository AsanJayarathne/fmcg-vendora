...
import { ShieldCheck, ArrowRight, CheckCircle2, AlertCircle, RefreshCw, Loader2, X, Mail } from "lucide-react";
      ...
      >
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-xs text-indigo-200/80 mt-1.5 leading-relaxed">
            {success ? (
              "Your distributor account has been successfully created."
            ) : (
              <>
        ...
        {!success && (
          <form onSubmit={handleVerify} className="space-y-6">
              ...
              {otp.map((digit, index) => (
                <input
                  ...
                  className={`w-11 h-13 sm:w-12 sm:h-14 text-center text-xl font-bold rounded-xl border transition-all outline-none ${
                    digit
                      ? "bg-indigo-600/20 border-indigo-500 text-white shadow-xs shadow-indigo-500/20"
                      : "bg-white/5 border-white/10 text-white/90 focus:border-indigo-400 focus:bg-white/10"
                  }`}
                />
              ))}
            </div>
            ...
            <button
              ...
              className="w-full py-3.5 rounded-xl text-sm font-bold text-white bg-gradient-to-r from-indigo-500 to-violet-600 hover:from-indigo-600 hover:to-violet-700 shadow-md shadow-indigo-500/20 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
                <>
            ...
            <div className="text-center pt-2">
              <p className="text-xs text-indigo-300/60 mb-2">Didn't receive the code?</p>
              <button
                type="button"
                onClick={handleResendOtp}
                disabled={resendCooldown > 0 || resending}
                className="text-xs font-bold text-indigo-400 hover:text-indigo-300 transition inline-flex items-center gap-1.5 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              >
