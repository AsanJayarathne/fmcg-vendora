...
function MyRoute() {
  ...
  const handleAction = async (id, action, routeItem) => {
    ...
    try {
      ...
      alert(err.message || `Error updating delivery status to ${action}`);
    } finally {
      setUpdatingId(null);
    }
  };

  // Filter routes according to activeTab, search query, and paymentFilter
  const filteredRoutes = useMemo(() => {
    return routes.filter((item) => {
      if (activeTab === 'Picked Up' && item.status !== 'Pending') return false;
      if (activeTab === 'Delivered' && item.status !== 'Delivered') return false;
      if (activeTab === 'Returned' && item.status !== 'Returned') return false;

      if (paymentFilter !== 'ALL') {
        if (item.rawPaymentMethod !== paymentFilter) return false;
      }
      ...
      if (searchQuery.trim()) {
        ...
        const matchesId = `ORD-${item.id}`.toLowerCase().includes(query) || String(item.id).includes(query);
    ...
    <div className="space-y-6 animate-fadeIn max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">Manage assigned deliveries, track split payments, and confirm completions</p>
        </div>
        <button
          onClick={fetchRoutes}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 sm:gap-4">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-xs flex items-center gap-3.5">
          ...
          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Cash to Collect</span>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-xs flex items-center gap-3.5">
          ...
          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Completed Shift Jobs</span>
            <div className="text-lg sm:text-xl font-extrabold text-slate-800">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-xs space-y-3.5">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
          ...
          <div className="bg-slate-100/80 p-1 rounded-xl border border-slate-200/50 flex gap-1 w-full lg:max-w-md">
          ...
          <div className="relative flex-1 w-full lg:max-w-md">
            ...
            <input
              type="text"
              placeholder="Search by store, address, or ID..."
              value={searchQuery}
