...
export default function ResetPassword() {
  ...
  useEffect(() => {
    if (!token || !email) {
      ...
      setError(t("auth.missingResetToken", "Missing reset token or email address in the link."));
      return;
    }
    ...
    const verifyToken = async () => {
      try {
        ...
        if (data.success && data.data?.valid) {
          ...
          setIsTokenValid(false);
          setError(data.message || t("auth.invalidOrExpiredLink", "This password reset link is invalid or has expired."));
        }
      } catch (err) {
        setIsTokenValid(false);
        setError(t("auth.networkError", "Network error — unable to verify reset token."));
      } finally {
        setVerifying(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!data.success) {
        setError(data.message || t("auth.failedResetPassword", "Failed to reset password. Please try again."));
        setLoading(false);
        return;
      }
    ...
    <div className="min-h-screen bg-[#2446D8] flex items-center justify-center p-4 sm:p-6 relative">
      ...
      <div className="bg-white rounded-[32px] w-full max-w-xl p-8 sm:p-12 shadow-2xl relative border border-blue-100">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <h2 className="text-2xl font-bold text-slate-800">{t("auth.setNewPassword", "Set New Password")}</h2>
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            {t("auth.chooseSecurePasswordFor", "Choose a secure password for")} <span className="font-semibold text-slate-700">{email || "your account"}</span>
          </p>
        </div>
        ...
        {verifying && (
          <div className="py-14 text-center">
            ...
            <p className="text-sm font-semibold text-slate-600">{t("common.loading", "Verifying secure reset link...")}</p>
          </div>
        )}
        ...
        {!verifying && !isTokenValid && (
          <div className="mt-8 text-center py-4">
            ...
            <h3 className="text-lg font-bold text-slate-800">{t("auth.linkInvalidTitle", "Reset Link Expired or Invalid")}</h3>
            <p className="text-xs sm:text-sm text-slate-500 mt-1.5 max-w-sm mx-auto leading-relaxed">
              {error || t("auth.linkExpiryNotice", "For your security, reset links expire after 15 minutes or can only be used once.")}
            </p>
        ...
        {!verifying && isTokenValid && success && (
          <div className="mt-8 text-center py-6">
            ...
            <h3 className="text-xl font-bold text-slate-800">{t("auth.passwordUpdated", "Password Updated Successfully!")}</h3>
            <p className="text-sm text-slate-500 mt-2">
              {t("auth.passwordUpdatedSubtitle", "Your password has been changed and all old sessions were securely signed out.")}
            </p>
            <p className="text-xs text-blue-600 font-semibold mt-3 animate-pulse">
              {t("auth.redirectingToLogin", "Redirecting you to Login in a moment...")}
            </p>
          </div>
        )}
        ...
        {!verifying && isTokenValid && !success && (
          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            ...
            <div className="bg-slate-50 border border-slate-100 rounded-xl p-3.5 space-y-1.5 text-xs text-slate-600">
              <p className="font-semibold text-slate-700 mb-1">{t("auth.passwordReqs", "Password Requirements:")}</p>
              <div className="flex items-center gap-2">
                ...
                <span className={hasMinLength ? "text-green-700 font-medium" : "text-slate-500"}>{t("auth.reqMinChars", "At least 8 characters")}</span>
              </div>
              <div className="flex items-center gap-2">
                ...
                <span className={hasNumber ? "text-green-700 font-medium" : "text-slate-500"}>{t("auth.reqNumber", "Contains a number")}</span>
              </div>
              <div className="flex items-center gap-2">
                ...
                <span className={hasLetter ? "text-green-700 font-medium" : "text-slate-500"}>{t("auth.reqLetters", "Contains letters")}</span>
              </div>
            </div>
            ...
            >
              {loading ? (
                <>
                  ...
                  <span>{t("common.loading", "Updating Password...")}</span>
                </>
              ) : (
                <>
                  <span>{t("auth.setNewPassword", "Save New Password")}</span>
                  <ArrowRight size={20} />
                </>
              )}
            </button>
